import { LayoutDashboard, ScanLine, FileSearch, FileText, LogOut, ShieldCheck } from 'lucide-react';
...
const navItems: { id: Page; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { id: 'scanner', label: 'Scan Product', icon: ScanLine },
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto scrollbar-thin">
          ...
          {navItems.map((item) => {
            ...
            const active = current === item.id || (item.id === 'scanner' && current === 'result');
            return (
              <button
                key={item.id}
