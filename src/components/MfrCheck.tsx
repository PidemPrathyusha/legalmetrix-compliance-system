...
import { UploadCloud, ImageIcon, X, PackageSearch, ArrowRight, FileSearch, Sparkles, ScanLine } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {hasAnyImage && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-teal-50 border border-teal-200 text-teal-700 text-sm font-medium">
              <Sparkles size={15} />
              {images.length} label{images.length !== 1 ? 's' : ''} ready
            </div>
          )}
        </div>
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-navy-50 border border-navy-100">
          ...
          <p className="text-xs text-navy-500 leading-relaxed">
            Upload your front, back, and side label artwork. Our AI engine will check for mandatory declarations required under the Legal Metrology (Packaged Commodities) Rules — including MRP, net quantity, manufacturer details, and more.
          </p>
        </div>
          ...
          {currentImage ? (
            ...
            <div className="py-16 sm:py-24 px-6 text-center">
              ...
              <h3 className="text-xl font-display font-bold text-navy-900 mb-2">
                {dragging ? 'Drop your image here' : `Upload ${labelTypes.find((l) => l.id === activeLabel)?.label}`}
              </h3>
              <p className="text-sm text-navy-400 max-w-md mx-auto">
                Drag and drop the <span className="font-semibold text-navy-600">{labelTypes.find((l) => l.id === activeLabel)?.label}</span> artwork here, or click to browse
              </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
          <div className="flex items-center gap-3 text-sm text-navy-400">
            <FileSearch size={18} />
            {hasAnyImage
              ? 'Ready to check. Click to run the pre-compliance screening.'
              : 'Upload at least one label image to begin the pre-compliance check.'}
          </div>
          <button
            onClick={handleCheck}
            disabled={!hasAnyImage || checking}
            className="group px-8 py-4 rounded-2xl bg-gradient-to-r from-teal-500 to-teal-600 text-white text-base font-semibold hover:shadow-xl hover:shadow-teal-500/30 transition-all hover:scale-105 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-none flex items-center gap-3 min-w-[240px] justify-center"
          >
            {checking ? (
              <>
        ...
        {checking && (
          <div className="fixed inset-0 bg-navy-950/40 backdrop-blur-sm z-50 flex items-center justify-center animate-fade-in">
            <div className="bg-white rounded-2xl p-8 max-w-sm w-full mx-4 card-shadow-lg text-center">
              ...
              <h3 className="text-lg font-display font-bold text-navy-900 mb-2">Running Pre-Compliance Check</h3>
              <p className="text-sm text-navy-400 mb-4">Screening your label against Legal Metrology Rules...</p>
              <div className="space-y-2 text-left">
                {['Extracting text via OCR', 'Identifying mandatory declarations', 'Checking compliance rules', 'Calculating compliance score'].map((step, i) => (
